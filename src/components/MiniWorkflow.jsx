...
export default function MiniWorkflow() {
  ...
  const [packetStage, setPacketStage] = useState("idle"); // idle | toHub | fromHub
  ...
  useEffect(() => {
    ...
    const runFlow = (idx) => {
      ...
      // 4. Pacote do hub para destinos
      t(() => {
        setPacketStage("fromHub");
      }, 2400);
  ...
  if (hubActive || activeDestinations.length > 0) {
    activeDestinations.forEach((d) => activeConnections.add(`hub-${d}`));
    if (packetStage === "fromHub") {
            ...
            {triggers.map((tr) => {
              ...
              return (
                <g key={`c-${tr.id}`}>
                  ...
                  />
                  {/* Pacote em trânsito */}
                  {packetStage === "toHub" && activeTrigger === tr.id && (
                    <g>
                      <circle r="8" fill="url(#packetGlow)">
              ...
              return (
                <g key={`c-${dest.id}`}>
                  ...
                  {packetStage === "fromHub" && (
                    <g>
                      <circle r="8" fill="url(#packetGlow)">
                        <animateMotion dur="1.4s" repeatCount="1" path={path} />
                      </circle>
                      <circle r="3.5" fill="#2563eb">
                        <animateMotion dur="1.4s" repeatCount="1" path={path} />
                      </circle>
          ...
          <div style={styles.logLines}>
            {logLines.map((line, i) => (
              <div
                ...
                style={{
                  ...styles.logLine,
                  color: line.startsWith("✓") ? "#16a34a" : "#475569",
                  animation: "ffLogIn 0.3s ease both",
                }}
              >
                {line}
              </div>
            ...
            {logLines.length === 0 && (
              <div style={{ ...styles.logLine, color: "#94a3b8" }}>
                Aguardando trigger...
              </div>
            )}
          </div>
